...
| `custom-permissions:write` | Optional | Room; user | Grants an isolated-list item's `additionalReaders`/`additionalEditors` to a custom permission in `custom-permissions-panel.tsx` via `mcpapp.rooms.customPermissions.setItemAccess`, and mints/assigns a demo permission via `app.rest` `rooms.customPermissions.create` / `.assign`. The Hub still requires the acting user to be room owner/admin and every id to exist in the room catalog. | The Custom permissions tab can read but cannot mint, assign, or grant item access. |
| `users:read` | Optional | Room; user | Resolves a typed member username to its user id in `custom-permissions-panel.tsx` via `app.rest` `users.info`, so a permission can be assigned by username instead of a raw user id. | The Custom permissions tab cannot assign a permission by username; mint and item grant still work. |
