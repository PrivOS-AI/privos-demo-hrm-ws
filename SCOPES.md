...
| `bot:credential:issue` | Optional | Workspace; user | Issues/rotates the installation bot's Hub credential in `bot-workload-panel.tsx`. The raw value is never rendered, logged, or stored — see `extractIssuanceEvidence` in `bot-workload-helpers.ts`. | Credential issuance is disabled; the bot cannot authenticate to Hub REST as itself. |
